...
map.on("load", async () => {
  async function parseCSVAndAddToMap2() {
    ...
    const image = await map.loadImage(
      "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d6/Map_icons_by_Scott_de_Jonge_-_fire-station.svg/1024px-Map_icons_by_Scott_de_Jonge_-_fire-station.svg.png"
    );
...
popup.setLngLat(coordinates).setHTML(`
 <div style="
    ...
    border-radius: 5px;
    padding: 10px;">
    <p style="margin: 0; font-weight: bold;">Fire ID</p>
    <p style="margin: 0;">${field_system_fire_id}</p>
    <hr>
...
async function parseCSVAndAddToMap() {
  const response = await fetch("./Fire_Stations_Canada.csv"); // Adjust path as needed
